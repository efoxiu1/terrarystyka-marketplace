...
export async function POST(req: Request) {
  try {
    ...
    // 4. Budujemy treść maila
    if (table === 'profiles' && record.is_banned === true) {
      ...
      // Ignorujemy paczki, które nas nie interesują (np. zmiana avatara)
      return NextResponse.json({ message: 'Zignorowano' }, { status: 200 });
    }
