    ...
    <main className="min-h-screen bg-white pb-20 overflow-x-hidden w-full">
      ...
      <section className={`border-b border-gray-100 pt-8 md:pt-16 pb-8 px-5 md:px-6 ${profile.is_banned ? 'opacity-50 pointer-events-none' : ''}`}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
          ...
          {/* Kontener na zdjęcie i teksty (ułożone obok siebie na większych ekranach) */}
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          
          {/* 1. CZYSTY AVATAR (bez absolutnych pozycji) */}
          <div className="w-28 h-28 md:w-36 md:h-36 rounded-full overflow-hidden shrink-0 border border-gray-200 bg-gray-50 flex items-center justify-center text-4xl shadow-sm">
            {profile?.avatar_url ? (
          ...
          {/* 2. DANE UŻYTKOWNIKA I ODZNAKA */}
          <div className="flex flex-col items-center sm:items-start pt-2">
            
            {/* Nick użytkownika */}
            <h1 className="text-2xl md:text-3xl font-black text-gray-900">
              {profile?.username || 'Nieznany Hodowca'}
            </h1>

            {/* Lśniąca odznaka Zweryfikowanego Sprzedawcy */}
            {profile?.is_verified_seller && (
              <div className="mt-2 inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full text-sm font-bold border border-blue-200 shadow-sm animate-in fade-in zoom-in duration-300">
            ...
            {/* Tu możesz dorzucić np. datę dołączenia czy miasto */}
            <p className="mt-3 text-sm text-gray-500 font-medium">
              Na giełdzie od {new Date(profile?.created_at).toLocaleDateString('pl-PL')}
            </p>
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 md:gap-3 mt-4">
                ...
                {profile.facebook_url && (
                  <a href={profile.facebook_url} target="_blank" rel="noopener noreferrer" 
                     className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl text-xs md:text-sm font-bold transition-all shadow-sm">
                ...
                {profile.instagram_url && (
                  <a href={profile.instagram_url} target="_blank" rel="noopener noreferrer" 
                     className="flex items-center gap-2 px-4 py-2 bg-pink-50 text-pink-600 hover:bg-pink-600 hover:text-white rounded-xl text-xs md:text-sm font-bold transition-all shadow-sm">
                ...
                {profile.youtube_url && (
                   <a href={profile.youtube_url} target="_blank" rel="noopener noreferrer" 
                      className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-xl text-xs md:text-sm font-bold transition-all shadow-sm">
